...
namespace Jumper1.Models
{
   public class CollusionManager
   {
      ...
      public bool HasCollided(uint id)
      {
         ...
         foreach (Collider c in colliders)
         {
            if (collider != c)
            {
               ...
               {
                  if (collider.MoveDirection == EColliderMoveDirection.Forward)
                  {
                     if (((collider.Vertex2x >= c.Vertex1x) &&
                          (collider.Vertex2x <= c.Vertex2x)) &&
                         ((collider.Vertex1y >= c.Vertex1y) &&
                         (collider.Vertex1y <= c.Vertex3y)))
                     {
                        hasCollided = true;
                        break;
                     }
                  ...
                  else if (collider.MoveDirection == EColliderMoveDirection.Backwards)
                  {
                     if (((collider.Vertex1x <= c.Vertex2x) &&
      ...
      {
         Collider collider = colliders.Find(c => c.Id == id);
         collider.Update(id, moveDirection, vertex1x, vertex1y, vertex2x, vertex2y, vertex3x, vertex3y, vertex4x, vertex4y);
      }
   }
}
